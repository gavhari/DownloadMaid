...
fn test_full_workflow() {
    ...
    File::create(test_dir.join("doc.pdf"))
        .unwrap()
        .write_all(b"pdf")
        .unwrap();
    File::create(test_dir.join("photo.jpg"))
        .unwrap()
        .write_all(b"jpg")
        .unwrap();
    File::create(test_dir.join("archive.zip"))
        .unwrap()
        .write_all(b"zip")
        .unwrap();
    File::create(test_dir.join("README"))
        .unwrap()
        .write_all(b"readme")
        .unwrap();
...
fn test_dry_run_no_changes() {
    ...
    File::create(test_dir.join("test.pdf"))
        .unwrap()
        .write_all(b"pdf")
        .unwrap();
    File::create(test_dir.join("photo.jpg"))
